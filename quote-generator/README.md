# ✨ Quote Generator

A simple Quote Generator built with **HTML**, **CSS**, and **JavaScript**.

Click the **"Get New Quote"** button to display a random motivational quote from a predefined collection. This project was built to practice JavaScript fundamentals such as DOM manipulation, arrays, event listeners, and random number generation.

---

## 🚀 Features

- Display a random motivational quote
- Responsive and clean UI
- Smooth button hover effect
- Built using vanilla JavaScript
- Beginner-friendly project 

---

## 🛠️ Technologies Used

- HTML5
- CSS3 (Flexbox)
- JavaScript (ES6)

---

## 📂 Project Structure

```
Quote-Generator/
│
├── index.html
├── styles.css
├── script.js
└── README.md
```

---

## 🧠 What I Learned

While building this project, I practiced:

- Selecting HTML elements using `getElementById()`
- Handling button clicks with `addEventListener()`
- Storing data in arrays
- Generating random array indexes using `Math.random()` and `Math.floor()`
- Updating webpage content using `textContent`
- Styling layouts using Flexbox

---

## ▶️ How to Run

1. Clone this repository.

```bash
git clone https://github.com/RamitSarker29/quote-generator.git
```

2. Open `index.html` in your browser.

No installation or dependencies required.

---

## 💡 Future Improvements

- Prevent the same quote from appearing twice in a row
- Add categories (Motivation, Success, Life, Study)
- Fetch quotes from a public API
- Add fade animations between quotes
- Add a "Copy Quote" button
- Add a "Share Quote" feature

---

## 📄 Author
**Ramit Sarker**
